...
class Display: UIView {
    ...
    private func drawTexture() {
        ...
        guard let drawable: CAMetalDrawable = metalLayer.nextDrawable(), let texture: MTLTexture = self.texture else { return }
    ...
    func currentCGImage(viewportSize: CGSize) -> CGImage? {
        ...
        guard let texture = lastDisplayedTexture else { return nil }
        ...
        let result = CGImage(width: width, height: height, bitsPerComponent: 8, bitsPerPixel: 32, bytesPerRow: rowBytes, space: colorSpace, bitmapInfo: bitmapInfo, provider: provider, decode: nil, shouldInterpolate: true, intent: CGColorRenderingIntent.defaultIntent)
