...
class YOLO {
    ...
    public func computeBoundingBoxes(features: MLMultiArray) -> [Prediction] {
        ...
        /*
         ...
         predict 5 bounding boxes (boxesPerCell).
         A bounding box consists of five data items: x, y, width, height,and a confidence score. Each grid
         cell also predicts which class each bounding box belongs to.
         
         The "features" array therefore contains (numClasses + 5) * boxesPerCell values for each grid cell.
         i.e. 125 channels. The total features array contains 125x13x13 elements.
            ...
            for cx in 0..<gridWidth {
                for b in 0..<boxesPerCell {
                    ...
                    */
                    ...
                    /*
                     The predicted tx and ty coordinates are relative to the location of the grid cell;
                     we use the logistic sigmoid to constrain these coordinates to the range 0 - 1.
                     Then we add the cell coordinates (0-12) and multiply by the number of pixels per grid cell (32).
                     Now x and y represent center of the bounding box in the original 416x416 image space.
                     */
                    ...
                    /*
                     ...
                     We use the logistic sigmoid to turn into a percentage.
                     */
...
//                        classes[c] = features[[channel+5+c, cy, cx] as [NSNumber]].floatValue
                    ...
                    /*
                     ...
                     how likely it is that there is an object in this box (but not what kind of object
                     it is), with the largest class prediction, which tells us what kind of object it detected
                     (but not where).
                     */
                    let confidenceInClass = bestClassScore * confidence
                    
                    /*
                     Since we compute 13x13x5 = 845 bounding boxes, we only want to keep
                     the ones whose combined score is over a certain threshold.
                     */
        ...
        /*
         ...
         But there still may be boxes that overlap too much with others.
         We'll use 'non-maximum suppression' to prune those duplicate bounding boxes.
         */
