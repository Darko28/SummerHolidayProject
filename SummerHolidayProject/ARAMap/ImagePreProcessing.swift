...
struct ImageProcessingConstants {
    ...
    static let optimalDimensionX: Int = 100
    static let optimalDimensionY: Int = 100
    // For Edge Detection
    static let intensityThreshold: Double = 120
}
...
class ImagePreProcessing {
    ...
    private func RGBAlphaMake(r: UInt32, g: UInt32, b: UInt32, alpha: UInt32) -> UInt32 {
        return mask8(x: r) | mask8(x: g<<8) | mask8(x: b<<16) | mask8(x: alpha<<24)
    }
    
    public func preProcessImage(image: UIImage) -> (UIImage, [Double]) {
        
        var grayScalePixels: [Double] = []
        // Resize the image
        let smallSizeImage = resizeImage(image: image, targetSize: CGSize(width: 2 * 100.0, height: 2 * 100.0))
        ...
        if let grayImage = grayScaleImage(image: smallSizeImage) {
            ...
            return (grayImage, grayScalePixels)
        }
        
        return (smallSizeImage, grayScalePixels)
    }
    ...
    private func grayScaleImage(image: UIImage) -> UIImage? {
        ...
        image.draw(in: rect, blendMode: CGBlendMode.luminosity, alpha: 1.0)
...
extension ImagePreProcessing {
    ...
    // MARK: - Canny Edge Detection Algo.
    ...
    private func cannyEdgeDetectionOperator(inputMatrix: [[Double]]) -> [[Double]] {
        ...
        printMatrix(inputMatrix: suppressedMatrix, name: "Suppression")
    ...
    private func sobelOperator(inputMatrix: [[Double]]) -> [[Double]] {
        ...
        for _ in 0..<ImageProcessingConstants.optimalDimensionX {
            ...
            filteredOutput.append(temArr)
        }
        
        // Calculate with Sobel Filter (3*3 matrix)
        
        let sobelFilterHorizontal: [[Double]] = [[1,0,-1], [2,0,-2], [1,0,-1]]
        ...
        for i in 1..<ImageProcessingConstants.optimalDimensionX-1 {
            for j in 1..<ImageProcessingConstants.optimalDimensionY-1 {
                ...
                var Gx = inputMatrix[i-1][j-1] * sobelFilterHorizontal[0][0]
                Gx = Gx + inputMatrix[i-1][j+1] * sobelFilterHorizontal[0][2]
                Gx = Gx + inputMatrix[i][j-1] * sobelFilterHorizontal[1][0]
                Gx = Gx + inputMatrix[i][j+1] * sobelFilterHorizontal[1][2]
                Gx = Gx + inputMatrix[i+1][j-1] * sobelFilterHorizontal[2][0]
                Gx = Gx + inputMatrix[i+1][j+1] * sobelFilterHorizontal[2][2]
    ...
    private func createImageFromRGBData(inputMatrix: [Double], width: Int, height: Int) -> UIImage {
        ...
        let bytesPerRow = bytesPerPixel * width
        let bitsPerComponent = 8    // rgba, so total length = 32bits
    ...
    class func buffer(from image: UIImage) -> CVPixelBuffer? {
        ...
        let attrs = [kCVPixelBufferCGImageCompatibilityKey: kCFBooleanTrue,
                     kCVPixelBufferCGImageCompatibilityKey: kCFBooleanTrue] as CFDictionary
