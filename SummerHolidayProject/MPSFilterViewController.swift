...
class MPSFilterViewController: UIViewController, ARSessionDelegate, RenderDelegate, ShutterButtonDelegate, UIGestureRecognizerDelegate {
    ...
    override func viewDidLoad() {
        ...
        // configure renderer
        renderer = FilterRenderer(session: session, metalDevice: context.device)
        renderer.drawRectResized(size: view.bounds.size)
        renderer.delegate = self
        
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(MPSFilterViewController.handleTap(gestureRecognizer:)))
