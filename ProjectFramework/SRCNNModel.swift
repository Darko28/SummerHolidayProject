...
public class SRCNNModel {
    
    static let shared = SRCNNModel()
    ...
    private func crop(src: UIImage) -> [PatchIn] {
        ...
        guard let cgImage = src.cgImage else { return [] }
    ...
    private func render(patches: [PatchOut], size: CGSize) -> UIImage? {
        ...
        for patch in patches {
            ...
            let rect = CGRect(x: pos.x * CGFloat(patchOutSize), y: pos.y * CGFloat(patchOutSize), width: CGFloat(patchOutSize), height: CGFloat(patchOutSize))
            image.draw(in: rect)
        }
    ...
    func convert(from src: UIImage) -> UIImage? {
        ...
        guard let resized = resize2x(src: src) else { return nil }
        let t0 = Date()
        print("resize: \(t0.timeIntervalSince(t))")
        
        guard let expanded = expand(src: resized) else { return nil }
        let t1 = Date()
